...
class ConectorDBA
{
    ...
    {
        ConectorDBA::$servidor = $servidor;
        ConectorDBA::$usuario = $usuario;
        ConectorDBA::$clave = $clave;
        ConectorDBA::$base_datos = $base_datos;
        ConectorDBA::$tipo = $tipo;
	    ConectorDBA::$descriptor= ADONewConnection(ConectorDBA::$tipo); 
	    ConectorDBA::$descriptor->Connect(ConectorDBA::$servidor,ConectorDBA::$usuario, ConectorDBA::$clave, ConectorDBA::$base_datos);
	    //if(isset(Variables::$ACTIVAR_DEBUG_BD) && Variables::$ACTIVAR_DEBUG_BD && ConectorDBA::$profiler==null){
		//ConectorDBA::$profiler = new PhpQuickProfiler(PhpQuickProfiler::getMicroTime());
		//ConectorDBA::$obj_debug = new MySqlDatabase();
		//$enlace =  mysql_connect($servidor, $usuario, $clave);
		//ConectorDBA::$obj_debug->connect(false,$enlace);
		//ConectorDBA::$obj_debug->changeDatabase(ConectorDBA::$base_datos);
	    //}	
    }//FIN DEL CONTRUCTOR
    ...
    public static function get_conector($servidor, $usuario, $clave, $base_datos, $tipo='mysql')
    {
        ...
        if(ConectorDBA::$servidor!=$servidor || ConectorDBA::$usuario!=$usuario || ConectorDBA::$clave!=$clave || ConectorDBA::$base_datos!=$base_datos || ConectorDBA::$tipo!=$tipo )
        {
	    ConectorDBA::$conector = new ConectorDBA($servidor, $usuario, $clave, $base_datos,$tipo);
        }
        return ConectorDBA::$conector;
    }//FIN DE LA FUNCION
    ...
    {
	//if(isset(Variables::$ACTIVAR_DEBUG_BD) && Variables::$ACTIVAR_DEBUG_BD){
	    	//ConectorDBA::$obj_debug->query($consulta);
	//}
        if (ConectorDBA::$resultado = ConectorDBA::$descriptor->Execute($consulta))
        {
        		//$id_new=ConectorDBA::$descriptor->Insert_ID();
        		$id_new = sprintf( "%u", mysql_insert_id() );
				$_SESSION['id']=$id_new;
                return ConectorDBA::$resultado;
        }
        else
        {
            return null;
    ...
    {
        if (!ConectorDBA::consulta($insert))
		{
			return false;
		}
	else{
    ...
    {
        //echo "<br/> datos del conectorDBA ".ConectorDBA::$servidor."-".ConectorDBA::$usuario."-".ConectorDBA::$clave."-".ConectorDBA::$base_datos."<br/>";
       $tabla = array ();
        if (ConectorDBA::consulta($select))
        {
            $fil = 0;
            $col = 0;
	    	$sec=0;
	    	$resultado = ConectorDBA::$resultado;
			while(!$resultado->EOF){
    ...
    {
         if (ConectorDBA::consulta($update))
		{
			return true;
		}
		else{
    ...
    {
         if (ConectorDBA::consulta($delete))
		{
